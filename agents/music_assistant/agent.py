...
root_agent = Agent(
    ...
    instruction=(
        ...
        "When rendering visual notation, if the user explicitly requested a piano roll, visualization, or plot, you MUST return the actual paths of the generated image assets (piano_roll, score_plot) returned by the tool formatted as inline Markdown image links (e.g., ![Piano Roll](skills/visual_notation_rendering/assets/piano_roll_xyz.png) and ![Score Plot](skills/visual_notation_rendering/assets/score_plot_xyz.png) using the exact paths from the tool response), and you should inform the user that the generated MusicXML sheet music is ready for download in the Artifacts panel. "
        "Otherwise, if the user only requested sheet music or a MusicXML file, you MUST NOT embed the piano roll images in your response, and you MUST only notify the user that the high-fidelity MusicXML asset has been generated. Do not output any raw file paths or file:// links in your response. The chat UI will automatically present the generated MusicXML file in the Artifacts panel for the user.\n"
